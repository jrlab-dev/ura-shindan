...
(function (root, factory) {
  ...
  function callback(data, now = new Date()) {
    ...
    item.used = true; item.usedCount = (item.usedCount || 0) + 1; item.lastUsedAt = stamp(now); history.callbackIds = [...(history.callbackIds || []),item.id].slice(-2); p.lastCallbackAt = stamp(now); return {kind:'week-callback',memoryId:item.id,text:item.type === 'play' ? `前に${item.value}で遊んだね` : `前に「${item.value}」って教えてくれたね`,sound:'happy'};
  }
