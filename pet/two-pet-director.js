...
(function (root, factory) {
  ...
  'use strict';
  const SECOND=1000, MINUTE=60000, MAX=5; let serial=0;
  ...
  function createSession(options){ const o=options||{}, t=now(o.now,Date.now()), seeded=rng(o.seed); const q=()=>o.rng?o.rng():seeded(); return {startedAt:t,actionCount:0,inviteCount:0,usedSceneIds:[],activeScene:null,token:`two-${t}-${++serial}`,_rng:q,nextInviteAt:t+20000,nextInviteActionCount:1}; }
  ...
  function canInvite(session,c){ if(!session||blocked(c)||session.activeScene||session.inviteCount>=MAX)return false; const t=now(c&&c.now,Date.now()); return session.actionCount>0 && t>=session.nextInviteAt && t>=session.startedAt+20000 && eligible(session,c).length>0; }
  function invite(session,c){ if(!canInvite(session,c))return null; const list=eligible(session,c), scene=list[Math.floor((session._rng?session._rng():0)*list.length)%list.length], t=now(c&&c.now,Date.now()), word=safeWord(c); const copy=JSON.parse(JSON.stringify(scene)); copy.steps.forEach(step=>{step.text=step.text.replace(/\{\{word\}\}/g,word);}); session.activeScene={...copy,invitedAt:t,expiresAt:t+copy.durationMs,token:session.token}; session.usedSceneIds.push(scene.id); session.inviteCount++; session.nextInviteAt=t+90*SECOND; session.nextInviteActionCount=session.actionCount+1; return {events:[{type:'invite',sceneId:scene.id,scene:copy}]}; }
