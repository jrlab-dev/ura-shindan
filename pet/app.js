(() => {
  ...
  function activePetId() { return Life && Life.getActivePetId ? Life.getActivePetId(state.data, activeProfileId()) : 'pet-1'; }
  ...
  function openPlayMenu() { if (!state.data || state.echoSession || state.game || Life.isSafetyPaused(state.data)) return; cancelTwoPetMoment('play-menu'); cancelCompanionMoment(); $('play-menu-dialog').showModal(); }
