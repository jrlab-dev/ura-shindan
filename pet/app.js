(() => {
  ...
  function renderTwoPets() { if (!state.data) return; const relation = pairRelation(); const pets = state.data.pets || []; const active = visualPetId(); ['pet-1','pet-2'].forEach(id => { const info = pets.find(item => item.id === id) || petInfo(id); const value = Life && Life.getProfilePetState ? Life.getProfilePetState(state.data, activeProfileId(), id) : null; const label = $(`${id}-name`); if (label) label.textContent = info.name; const node = petNode(id); const slot = node && node.closest('.pet-slot'); if (slot) slot.classList.toggle('is-active-pet', id === active); if (node) { const presentation=twoPetPresentation(id); node.dataset.presentation=presentation.visual||''; node.dataset.active = String(id === active); node.setAttribute('aria-label', value ? `${info.name}。気分${value.mood}、体力${value.energy}、表情${value.lastExpression}${id === active ? '、いま遊ぶ主役' : '、見守り中'}` : `${info.name}がこちらを見ている`); const isSleeping = Boolean(Life && Life.isPetSleeping && Life.isPetSleeping(id, new Date())); const isActiveInSession = state.echoSession && (state.echoSession.imitatorPetId === id || id === active); const isCopyState = String(node.dataset.state || '').indexOf('copy-') === 0; if (isSleeping) { node.dataset.state = 'sleepy'; const sleepBubble = $(`${id}-bubble`); if (sleepBubble) { sleepBubble.hidden = true; sleepBubble.textContent = ''; } } else if (!isActiveInSession && !isCopyState && node.dataset.state === 'sleepy') node.dataset.state = 'normal'; } }); const guestSleeping = Boolean(Life && Life.isPetSleeping && Life.isPetSleeping('pet-2', new Date())) && active !== 'pet-2'; const showSecond = Boolean(relation && relation.phase === 'cohabiting') || Boolean(state.twoPetGuestVisible) || active === 'pet-2' || guestSleeping; $('pet-2-slot').hidden = !showSecond; const stageNow = Life && Life.getStageState ? Life.getStageState(state.data, activeProfileId(), new Date()) : null; const allAsleep = stageNow && stageNow.allAsleep; const status = $('two-pet-status'); if (status) { status.hidden = !showSecond; const friendshipStage = relation && relation.friendshipStage || 'together'; const friendshipLabel = friendshipStage === 'not-met' ? 'まだ あってない' : friendshipStage === 'visiting' ? 'あそびにきた' : friendshipStage === 'familiar' ? 'なかよし' : 'ずっといっしょ'; status.textContent = allAsleep ? 'ふたりとも ねんねしているよ' : guestSleeping ? `${petInfo('pet-2').name} は うしろで ねんねしているよ` : relation && relation.phase === 'cohabiting' ? `いまは ${petInfo(active).name} が主役・${friendshipLabel}` : active === 'pet-2' ? `${petInfo(active).name} が主役だよ` : 'ふわが あそびにきたよ'; } const note = $('welcome-note'); if (note) { if (typeof state.welcomeNoteDefault !== 'string') state.welcomeNoteDefault = note.textContent; note.textContent = stageNow && stageNow.allAsleep ? (stageNow.pairAvailable ? 'ふたりとも ねてるよ' : `${petInfo('pet-1').name}は ねてるよ`) : state.welcomeNoteDefault; } if (stageNow && stageNow.allAsleep) { const mainBubble = $('bubble'); if (mainBubble) { mainBubble.hidden = true; mainBubble.textContent = ''; } } }
  function echoTuning(petId) { const presentation = twoPetPresentation(petId); const override = state.data && state.data.echoVoiceOverrides && state.data.echoVoiceOverrides[petId]; return override || { pitchRate:Number(presentation.pitchRate) || (petId === 'pet-2' ? .55 : 2.3), speedRate:Number(presentation.speedRate) || (petId === 'pet-2' ? .85 : 1.11), doubleMix:Number(presentation.doubleMix) || 0, brightness:Number(presentation.brightness) || (petId === 'pet-2' ? 35 : 70), timingMode:'preserve' }; }
