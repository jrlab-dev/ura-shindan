(() => {
  ...
  function renderTwoPets() { if (!state.data) return; const relation = pairRelation(); const pets = state.data.pets || []; const active = visualPetId(); ['pet-1','pet-2'].forEach(id => { const info = pets.find(item => item.id === id) || petInfo(id); const value = Life && Life.getProfilePetState ? Life.getProfilePetState(state.data, activeProfileId(), id) : null; const label = $(`${id}-name`); if (label) label.textContent = info.name; const node = petNode(id); const slot = node && node.closest('.pet-slot'); if (slot) slot.classList.toggle('is-active-pet', id === active); if (node) { const presentation=twoPetPresentation(id); node.dataset.presentation=presentation.visual||''; node.dataset.active = String(id === active); node.setAttribute('aria-label', value ? `${info.name}。気分${value.mood}、体力${value.energy}、表情${value.lastExpression}${id === active ? '、いま遊ぶ主役' : '、見守り中'}` : `${info.name}がこちらを見ている`); } }); const showSecond = Boolean(relation && relation.phase === 'cohabiting') || Boolean(state.twoPetGuestVisible) || active === 'pet-2'; $('pet-2-slot').hidden = !showSecond; const status = $('two-pet-status'); if (status) { status.hidden = !showSecond; status.textContent = relation && relation.phase === 'cohabiting' ? `いまは ${petInfo(active).name} が主役・なかよし ${relation.friendshipStage || 'together'}` : active === 'pet-2' ? `${petInfo(active).name} が主役だよ` : 'ふわが あそびにきたよ'; } }
